...
// This file provides a set of wrapper methods for the ExecutionStateStore type, enabling advanced operations on cached data paths.
...
/*
The following code defines a set of utility methods for the ExecutionStateStore type,
...
func (this *ExecutionStateStore) IndexOf(tx uint64, path string, key any, T crdtcommon.CRDT) (uint64, uint64) {
...
func (this *ExecutionStateStore) KeyAt(tx uint64, path string, index any, T crdtcommon.CRDT) (string, uint64) {
	if !common.IsPath(path) {
...
func (this *ExecutionStateStore) PeekCommitted(path string, T crdtcommon.CRDT) (any, uint64) {
	v, _ := this.committedStore.Get(path)
	return v, 0
}
...
func (this *ExecutionStateStore) getKeyByIdx(tx uint64, path string, idx uint64) (any, uint64, error) {
	if !common.IsPath(path) {
...
// get the key of the Nth element under a path
func (this *ExecutionStateStore) Min(tx uint64, path string, idx uint64) (any, uint64, error) {
	if !common.IsPath(path) {
		return nil, stgcommon.MIN_READ_SIZE, errors.New("Error: Not a path!!!")
	}
...
// get the key of the Nth element under a path
func (this *ExecutionStateStore) Max(tx uint64, path string, idx uint64) (any, uint64, error) {
	if !common.IsPath(path) {
		return nil, stgcommon.MIN_READ_SIZE, errors.New("Error: Not a path!!!")
	}
...
func (this *ExecutionStateStore) ReadAt(tx uint64, path string, idx uint64, T crdtcommon.CRDT) (any, uint64, error) {
...
// Read th Nth element under a path
func (this *ExecutionStateStore) PopBack(tx uint64, path string, T crdtcommon.CRDT) (any, int64, error) {
	if !common.IsPath(path) {
...
func (this *ExecutionStateStore) WriteAt(tx uint64, path string, idx uint64, T crdtcommon.CRDT) (int64, error) {
	if !common.IsPath(path) {
